...
export default async function ProductsPage({searchParams}:{searchParams:{[k:string]:string|string[]|undefined}}){
  ...
  const filtered=q?all.filter(p=>[p.title,p.vendor,p.type,(p.tags||[]).join(' '),(p.description||'')].filter(Boolean).join(' ').toLowerCase().includes(q)):all;
